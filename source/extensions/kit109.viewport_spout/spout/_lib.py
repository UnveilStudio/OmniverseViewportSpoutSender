...
if not os.path.exists(_DLL_PATH):
    # Fallback: locate the DLL in the original SpoutForPython directory,
    # navigating from the Kit app folder (${app} = _build/.../apps/).
    try:
        import carb.tokens as _carb_tokens
        _app_dir = _carb_tokens.get_tokens_interface().resolve("${app}")
        # ${app}/../../../.. == kit-app-template repo root
        _repo_root = os.path.abspath(os.path.join(_app_dir, "..", "..", "..", ".."))
        _candidate = os.path.join(
            _repo_root, "SpoutForPython", "SpoutForPython", "spout", "SpoutLibrary.dll"
        )
...
try:
    ...
    raise OSError(f"Failed to load SpoutLibrary.dll from '{_DLL_PATH}': {exc}") from exc
...
# the header exactly (confirmed empirically).
V_UNDOCUMENTED_98 = 98
...
V_GET_PATH_STR   = 105  # std::string arg — DO NOT CALL
V_START_TIMING   = 106
V_END_TIMING     = 107
...
# --- Graphics preference (145-150, requires NTDDI_WIN10_RS4) ---
V_GET_PERF_PREF    = 145
V_SET_PERF_PREF    = 146
